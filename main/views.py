...
def home(request):
    ...
    if request.session.has_key("username"):
        ...
        embedLink = "https://www.youtube.com/embed/" + dataFinal["link"].split("v=")[1]
        return render(
            request,
            "index.html",
            {
        ...
        if request.method == "POST":
            ...
            if not result_json.get("success"):
                ...
                try:
                    ...
                    dataid = str("".join(random.choices(string.ascii_uppercase, k=6)))
                    ...
                    entryInstance.districtCode = (
                        District.objects.filter(districtName=district)
                        .first()
                        .districtCode
                    )
                    ...
                    entryInstance.state = State.objects.filter(stateName=state).first()
                    entryInstance.district = District.objects.filter(
                        districtName=district, state=entryInstance.state
                    ).first()
                    entryInstance.taluka = Taluka.objects.filter(
                        talukaName=taluka, district=entryInstance.district
                    ).first()
                    entryInstance.city = City.objects.filter(
                        cityName=city, taluka=entryInstance.taluka
                    ).first()
        ...
        if request.method == "POST":
            ...
            if not result_json.get("success"):
                ...
                try:
                    ...
                    entryInstance.subYear = (
                        int(subYear) if subYear != "" else entryInstance.subYear
                    )
                    entryInstance.name = name
                    entryInstance.email = email
                    entryInstance.addressOne = address1
